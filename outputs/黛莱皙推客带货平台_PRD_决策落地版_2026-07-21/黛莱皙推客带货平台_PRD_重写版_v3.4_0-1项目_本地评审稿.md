...
| FR-MP-002 | 首页与搜索 | Banner、快捷频道、直播商品混排、热门短视频、首页悬浮窗、热门商品、商品 Feed、混合搜索 | P1 | G-001 |
...
| FR-MP-006 | 发圈 | 带货发圈、宣发、收藏 Feed、素材复制下载和商品分享 | P0 | G-001/G-005 |
| FR-MP-007 | 我的与收益 | 收益、业绩中心、业绩明细、手机号授权、签约、提现和记录 | P0 | G-003/G-005 |
...
| --- | --- | --- | --- | --- |
| 首页结构 | Logo、搜索、Banner、快捷频道、直播商品混排、热门短视频、首页悬浮窗、热门商品、商品 Feed | 商品分类 | 进入搜索、运营内容或商品详情 | 不展示独立直播入口、消息中心和业务分享入口 |
| Banner | 图片、标题、轮播指示 | 不适用 | 跳转后台配置目标 | 未配置目标时不可点击；无有效数据时整个模块隐藏 |
| 快捷频道 | 图标、名称 | 不适用 | 进入配置页面或商品分类 | 仅展示已启用且目标有效的频道 |
| 直播商品混排 | 左侧：卡片标题、“查看更多 >”、竖版直播封面、直播状态、观看人数、主播头像、主播昵称、预计收益“赚 xx”；右侧：商品卡标题、商品贴图 | 不适用 | 点击直播封面进入当前直播详情；点击“查看更多 >”进入直播列表；点击整张商品贴图进入对应商品详情 | “热门直播”和商品卡标题字号一致；仅展示直播管理中已上架且已推荐的直播，在同一封面位按人工排序自动轮播；主播头像、昵称和“赚 xx”以透明叠层直接渲染在封面底部；右侧不单独渲染商品封面、商品名称、售价或收益文字，商品视觉与“赚 xx”均属于贴图内容；右侧最多 2 个商品卡，每个商品卡固定展示 2 张商品贴图；直播和商品模块分别启停，无有效数据时隐藏对应区域 |
| 热门短视频 | 模块标题、“更多短视频 >”、竖版封面、播放标识、视频号名称、运营标题、预计收益“赚 xx” | 不适用 | 点击卡片直接进入对应视频号视频；点击“更多短视频 >”进入短视频列表 | 位于直播商品混排下方；横向滑动；仅展示已上架且已推荐的视频，按人工排序展示；取消推荐、下架或授权失效后自动移除；无有效数据时整个模块隐藏 |
| 首页悬浮窗 | 后台配置图标 | 不适用 | 点击后跳转后台配置地址 | 固定在首页内容区右侧、底部导航上方；仅首页展示；开始滑动时立即隐藏，停止滑动 300ms 后恢复；隐藏期间不可点击；停用、图标缺失或跳转地址无效时不展示 |
| 热门商品 | 商品图、标题、价格、预计收益、分享按钮、推荐理由 | 不适用 | 正文进详情；按钮进分享 | 后台选择并排序；不设置“查看更多” |
| 商品 Feed | 分类筛选、主图、标题、价格、预计收益 | 有效分类 | 切换分类、进入详情、分页加载 | 不展示“全部商品”区块标题；分类筛选栏直接位于商品列表上方；卡片不展示活动按钮 |
...
| 有效直播 | 有效商品卡 | 首页布局 |
| --- | --- | --- |
| 有 | 2 张 | 左侧直播轮播，右侧 2 张商品卡纵向排列 |
| 有 | 1 张 | 左侧直播轮播，右侧 1 张商品卡铺满右侧区域，不保留第二张空位 |
| 无 | 2 张 | 隐藏直播区域，2 张商品卡在整行横向平分 |
| 无 | 1 张 | 隐藏直播区域，1 张商品卡铺满整行 |
| 有 | 无 | 隐藏商品区域，直播卡铺满整行 |
| 无 | 无 | 隐藏整个直播商品混排模块，不保留空白占位 |
...
| AC-FR-MP-002-04 | 首页悬浮窗已启用且图标、跳转地址有效 | 用户打开首页并点击悬浮窗 | 首页右侧展示配置图标，点击后跳转对应地址；切换到其他一级页面后不展示 |
| AC-FR-MP-002-05 | 直播与商品卡均已启用，直播管理存在多场已上架且已推荐直播，商品卡配置 4 个有效商品 | 用户打开首页并点击不同区域 | 左侧按直播人工排序在同一竖版封面位自动轮播；右侧两张商品卡分别展示 2 张商品贴图，点击整张贴图进入对应商品详情 |
| AC-FR-MP-002-06 | 直播商品混排仅配置 1 个商品卡及其 2 个有效商品 | 用户打开首页 | 右侧单个商品卡填满商品区域并展示 2 张商品贴图，不展示第二张空卡或空白占位 |
| AC-FR-MP-002-07 | 直播管理存在多场已上架且已推荐直播 | 用户等待轮播或点击“查看更多 >” | 封面按直播人工排序自动切换且主播信息同步更新；“查看更多 >”进入直播列表 |
| AC-FR-MP-002-08 | 后台存在已上架且已推荐、未推荐或已下架的视频 | 用户打开首页热门短视频区域 | 仅展示已上架且已推荐的视频并按人工排序横向排列；点击卡片进入对应视频号视频 |
| AC-FR-MP-002-09 | 首页没有有效推荐直播，存在 2 张完整有效商品卡 | 用户打开首页 | 不展示直播空位，2 张商品卡在整行横向平分 |
| AC-FR-MP-002-10 | 首页没有有效推荐直播，仅存在 1 张完整有效商品卡 | 用户打开首页 | 单张商品卡铺满整行，不展示第二张空卡 |
| AC-FR-MP-002-11 | 首页存在有效推荐直播，没有有效商品卡 | 用户打开首页 | 直播卡铺满整行，不展示商品空位 |
| AC-FR-MP-002-12 | 首页悬浮窗处于有效展示状态 | 用户开始滑动首页内容并停止 | 滑动开始后悬浮窗立即隐藏且不可点击；停止滑动 300ms 后在原位置恢复 |
...
| 视频列表 | 封面、播放标识、运营标题、视频号名称、关联商品、商品价格、预计收益 | 视频标题、视频号名称；精选、个护美妆、食品饮料、家清日用、水果生鲜等商品一级分类 | 切换分类、直接进入视频号视频 | “精选”为全部有效视频的默认聚合页；其他分类由视频关联主推商品的一级分类自动确定；不建设小程序视频详情页和推广按钮；无法核验的播放量不展示 |

- **失效处理**：直播结束、视频下架、删除或授权失效后停止展示和跳转。
- **视频分类规则**：每条视频以唯一关联主推商品作为分类依据；商品类目变更后视频分类同步更新。未关联有效商品或无法识别类目的视频归入“其他”，分类下无内容时展示空状态。
...
| AC-FR-MP-005-04 | 视频分别关联不同一级类目的主推商品 | 切换视频商品分类 | 仅展示关联主推商品属于当前类目的视频；切回“精选”展示全部有效视频 |

计划测试：`MP-L-001～011`。

#### 6.1.6 FR-MP-006：带货发圈、宣发与收藏
...
| 页面 | 展示字段 | 搜索/筛选 | 操作 | 特殊规则 |
| --- | --- | --- | --- | --- |
| 页面结构 | 带货发圈、宣发、收藏频道和当前选中态 | 当前频道关键词、有效分类 | 切换频道 | 切换频道或筛选条件时重置分页 |
| 带货发圈 | 发布主体、时间、文案、图片、评论话术、绑定商品、价格、预计收益、收藏状态 | 文案、素材标题、商品关键词、分类 | 收藏/取消收藏、复制评论、分享绑定商品、下载素材 | 每张内容卡右上角固定显示收藏按钮；用户不可更换绑定商品 |
| 宣发 | 发布主体、时间、文案、图片或视频、收藏状态 | 文案、素材标题、分类 | 收藏/取消收藏、复制文案、下载素材 | 每张内容卡右上角固定显示收藏按钮；不展示商品、预计收益或商品分享 |
| 收藏 Feed | 已收藏内容、来源类型、原内容全部可用字段 | 当前频道关键词 | 取消收藏、执行原内容已有操作 | 合并带货发圈和宣发收藏，按最近收藏时间倒序；收藏后立即加入，取消后立即移除；不改变原内容类型和操作边界 |
| 素材有效性 | 当前发布版本、有效期和授权状态 | 不适用 | 操作前复核 | 撤回、过期或授权失效后停止复制、下载和分享 |

- **收藏规则**：收藏按当前微信小程序用户保存，不要求推广大使资格；同一内容只保留一条收藏关系。原内容撤回、下架、过期或授权失效后，不再在收藏 Feed 展示。
...
| AC-FR-MP-006-04 | 带货发圈或宣发内容尚未收藏 | 点击卡片右上角“收藏” | 按钮变为“已收藏”，内容立即出现在收藏 Feed 且不重复 |
| AC-FR-MP-006-05 | 内容已经收藏 | 在原 Feed 或收藏 Feed 点击“已收藏” | 收藏关系取消，原 Feed 按钮恢复为“收藏”，收藏 Feed 同步移除该内容 |
| AC-FR-MP-006-06 | 收藏 Feed 同时包含带货发圈和宣发 | 查看并操作收藏内容 | 按最近收藏时间倒序展示来源类型；带货内容保留分享好物，宣发仍不出现商品分享 |
...
- **优先级**：P0
- **原型定位**：我的、业绩中心、业绩明细、首次提现签约、提现、提现记录。
- **入口**：底部“我的”。

| 功能 | 展示字段 | 搜索/筛选 | 操作 | 特殊规则 |
| --- | --- | --- | --- | --- |
| 我的首页 | 头像、昵称、当前身份、收益卡、业绩中心及业务入口 | 不适用 | 切换业绩时间、进入各子页面 | 入口按身份、资格和有效配置展示 |
| 我的收益 | 可提现、累计、已提现、待结算及口径说明 | 不适用 | 隐藏/显示金额、进入提现 | 未登录、未开通或金额未知时显示“--”；待结算不可提现 |
| 业绩中心 | 总交易额、总推广订单、总收益 | 全部、今日、近7日、近30日 | 切换时间范围、进入业绩明细 | 三项指标共用同一时间范围并同步刷新；未登录、未开通或数据未知时显示“--” |
| 业绩明细 | 时间范围、汇总指标、订单号、商品、推广来源、交易金额、推广收益、订单状态和下单时间 | 全部、今日、近7日、近30日 | 切换时间范围、复制完整订单号 | 从业绩中心进入时继承当前时间范围；页面订单号默认脱敏；用户不可编辑或删除 |
...
| 指标 | 统计规则 | 特殊说明 |
| --- | --- | --- |
| 总交易额 | 对所选时间范围内有效推广归因订单的实付金额求和 | 取消、未支付和全额退款订单不计；部分退款按平台最终有效金额扣减 |
| 总推广订单 | 对所选时间范围内有效推广归因且已支付的订单去重计数 | 以订单号去重；取消、未支付和全额退款订单不计 |
| 总收益 | 汇总所选时间范围内推广订单当前有效收益；待结算使用平台返回的预计收益，已结算使用实际收益，已失效计 0 | 用于业绩统计，不等同于可提现余额；最终金额以平台结算结果为准 |

- **时间规则**：“今日”按用户当前自然日统计；“近7日”和“近30日”均包含当日，分别覆盖连续 7 个和 30 个自然日；“全部”统计该推客开通后的全部有效数据。
- **数据联动**：业绩中心与业绩明细使用同一数据源和统计口径；切换时间范围后，三项汇总指标和订单明细必须同时刷新。
...
| AC-FR-MP-007-06 | 用户在业绩中心选择任一时间范围 | 切换“全部/今日/近7日/近30日” | 总交易额、总推广订单和总收益按同一范围同步刷新 |
| AC-FR-MP-007-07 | 业绩中心已选择非默认时间范围 | 点击“查看明细” | 进入业绩明细并继承当前时间范围，汇总值与订单列表使用相同口径 |
| AC-FR-MP-007-08 | 订单发生退款、取消或失效 | 刷新业绩数据 | 总交易额、总推广订单和总收益按平台最终有效结果扣减，且不计入可提现余额 |
| AC-FR-MP-007-09 | 用户进入“我的”页面 | 查看功能入口 | 不再展示“佣金明细”入口；业绩订单统一从业绩中心进入 |

计划测试：`MP-M-001～036`、`SC-004～007、015`。
...
| 直播管理 | 直播间、直播状态、上架状态、开播时间、主推商品、推荐文案、分享话术、首页热门状态、人工排序 | 直播间/视频号；直播状态、上架状态 | 同步、上下架、推荐上热门/取消热门、配置主推商品/文案/话术/排序、查看或编辑详情 | 微信同步，不新增或删除；至少一条分享话术才可上架；仅已上架直播可推荐，下架同步取消热门推荐 |
| 视频管理 | 封面、标题、视频号、关联主推商品、商品一级分类、时长、上架状态、首页热门状态、人工排序、最近同步时间 | 视频标题/视频号/关联商品；上架状态、商品一级分类 | 同步、上下架、推荐上热门/取消热门、调整排序 | 微信同步，不新增或删除；商品类目自动继承关联主推商品；只展示可通过官方链路跳转的视频；仅已上架视频可推荐，下架同步取消热门推荐 |
...
| 直播热门推荐 | 首页热门状态、人工排序 | 推荐时必填排序 | 默认未推荐 | 仅已上架直播可推荐；推荐后进入首页热门直播轮播，取消推荐或下架后立即移出 |
| 视频商品分类 | 关联主推商品、商品一级分类 | 关联商品必填 | 关联商品来自视频同步结果；商品类目只读并自动带入 | 前端按关联主推商品的当前一级分类归类；商品类目变更后自动更新；无有效关联商品时归入“其他” |
| 视频排序 | 人工排序 | 否 | 未设置时排在已有人工排序之后 | 只影响后台上架视频的前端顺序 |
| 视频上下架 | 上架状态 | 是 | 当前状态 | 下架后前端停止展示和跳转 |
| 视频热门推荐 | 首页热门状态 | 是 | 默认未推荐 | 仅已上架视频可推荐；推荐后进入首页热门短视频，取消推荐或下架后立即移出 |
...
| AC-FR-AD-003-07 | 视频已上架但未推荐 | 点击“推荐上热门” | 视频进入首页热门短视频并按人工排序展示；取消推荐或下架后从首页移除 |
| AC-FR-AD-003-08 | 直播已上架但未推荐 | 点击“推荐上热门” | 直播进入首页热门直播并按人工排序轮播；取消推荐或下架后从首页移除 |
| AC-FR-AD-003-09 | 视频已关联主推商品且商品存在一级分类 | 同步视频或商品类目发生变更 | 视频管理显示关联商品和最新商品一级分类，小程序视频分类同步更新，无需人工重复维护 |
...
| 首页热门直播 | 启用状态、当前已推荐直播数量 | 页面内启停 | 内容来自“直播管理”中已上架且已推荐的直播；首页装修不重复选择直播 |
| 首页商品卡 | 商品卡标题、每卡 2 个商品及贴图、启用状态 | 页面内直接配置、启停 | 商品卡最多 2 个，每卡固定选择 2 个商品并上传 2 张贴图 |
| 首页热门短视频 | 启用状态、当前已推荐视频数量 | 页面内启停 | 内容来自“视频管理”中已上架且已推荐的视频；首页装修不重复选择视频 |
| 首页悬浮窗 | 图标、跳转地址、状态 | 页面内直接配置、启停 | 与直播、商品组件合并为“首页装修”页面；首页只保留一个悬浮窗配置；小程序滑动时自动隐藏 |

“首页装修”采用左右分栏：左侧仅保留悬浮窗配置、热门直播开关与推荐摘要、商品卡配置、热门短视频开关与推荐摘要，右侧为小程序首页实时预览区域；四部分共用一次保存。直播和短视频的推荐管理分别在直播管理、视频管理完成。
...
| 字段 | 必填 | 输入/默认 | 校验与联动 |
| --- | --- | --- | --- |
| 启用状态 | 是 | 默认启用 | 停用后首页隐藏直播区域且不保留占位 |
| 内容来源 | 不适用 | 自动读取直播管理的推荐结果 | 仅取已上架且已推荐直播；本页不提供直播选择器 |
| 展示顺序 | 不适用 | 使用直播人工排序 | 单场失效、取消推荐或下架时移除；无有效直播时隐藏直播区域 |

**首页商品卡配置字段**

| 字段 | 必填 | 输入/默认 | 校验与联动 |
| --- | --- | --- | --- |
| 商品卡 1 标题 | 是 | 人工输入 | 为空不得保存；长度上限待确认 |
| 商品卡 1 商品 A | 是 | 从有效已上架商品中单选 | 未选择不得保存 |
| 商品卡 1 贴图 A | 是 | 上传 JPG/PNG | 未上传不得保存；整张贴图点击后进入商品 A |
| 商品卡 1 商品 B | 是 | 从有效已上架商品中单选 | 未选择不得保存；不得与商品 A 重复 |
| 商品卡 1 贴图 B | 是 | 上传 JPG/PNG | 未上传不得保存；整张贴图点击后进入商品 B |
| 商品卡 2 标题 | 否 | 人工输入 | 配置商品卡 2 时必填 |
| 商品卡 2 商品 A | 否 | 从有效已上架商品中单选 | 配置商品卡 2 时必填 |
| 商品卡 2 贴图 A | 否 | 上传 JPG/PNG | 配置商品卡 2 时必填 |
| 商品卡 2 商品 B | 否 | 从有效已上架商品中单选 | 配置商品卡 2 时必填；4 个商品不得重复 |
| 商品卡 2 贴图 B | 否 | 上传 JPG/PNG | 配置商品卡 2 时必填 |
| 启用状态 | 是 | 默认启用 | 仅启用且商品配置完整有效时在首页展示 |

所选商品决定贴图的跳转目标，上传文件决定前台展示内容；商品视觉与“赚 xx”均需合成在贴图中，前台不再拆分展示商品封面、名称、售价或收益字段。
已推荐直播的封面、状态、观看人数、主播头像、主播昵称和预计收益由直播数据自动带入；轮播切换时上述信息同步更新，直播预计收益统一显示为“赚 xx”。

**首页热门短视频配置字段**

| 字段 | 必填 | 输入/默认 | 校验与联动 |
| --- | --- | --- | --- |
| 启用状态 | 是 | 默认启用 | 停用后首页隐藏整个模块且不保留占位 |
| 内容来源 | 不适用 | 自动读取视频管理的推荐结果 | 仅取已上架且已推荐视频；本页不提供视频选择器 |
| 展示顺序 | 不适用 | 使用视频人工排序 | 排序相同时按最近同步顺序处理 |

**首页悬浮窗配置字段**

| 字段 | 必填 | 输入/默认 | 校验与联动 |
| --- | --- | --- | --- |
| 悬浮窗图标 | 是 | 上传图片；建议使用透明底 PNG | 为空不得保存 |
| 跳转地址 | 是 | 小程序内部页面路径或平台支持的业务目标 | 保存前校验地址格式和目标有效性 |
| 启用状态 | 是 | 默认启用 | 仅启用且图标、跳转地址均有效时在首页展示 |
| 滚动显隐 | 不适用 | 系统固定规则 | 开始滑动立即隐藏，停止滑动 300ms 后恢复；隐藏期间不响应点击 |
...
| AC-FR-AD-008-06 | 首页悬浮窗缺少图标或跳转地址无效 | 保存配置 | 阻止保存并定位错误字段 |
| AC-FR-AD-008-07 | 首页悬浮窗已停用 | 用户打开首页 | 不展示悬浮窗且不保留空白占位 |
| AC-FR-AD-008-08 | 直播管理已推荐多场有效直播，首页热门直播已启用；商品卡 1 已填写标题、选择 2 个有效商品并上传对应贴图 | 保存首页装修 | 首页按直播人工排序自动轮播推荐直播，并展示 1 个含 2 张可点击贴图的商品卡 |
| AC-FR-AD-008-09 | 任一已配置商品卡未选满 2 个商品、未上传 2 张贴图，或不同商品位选择同一商品 | 保存配置 | 阻止保存并定位对应字段 |
| AC-FR-AD-008-10 | 运营人员进入“首页装修” | 修改悬浮窗、启停热门直播、编辑商品卡、启停热门短视频 | 左侧完成模块配置，右侧实时更新小程序预览；装修页不出现直播和视频选择器；点击一次“保存首页装修”统一保存四部分配置 |
| AC-FR-AD-008-11 | “首页装修”热门短视频已启用，视频管理存在已上架且已推荐的视频 | 运营人员查看右侧预览并保存 | 预览展示推荐视频；保存后首页按视频管理推荐结果展示，不要求在装修页再次选择视频 |
| AC-FR-AD-008-12 | 首页装修中的直播或商品卡无有效内容 | 运营人员查看右侧预览 | 预览按直播商品自适应规则重排，不显示空卡或无效占位 |
| AC-FR-AD-008-13 | 首页悬浮窗已启用 | 运营人员滚动右侧小程序预览 | 预览中的悬浮窗在滚动期间隐藏，停止滚动 300ms 后恢复 |
